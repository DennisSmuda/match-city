...
export async function checkGrid(x: number, y: number, chainable: boolean = false) {
  ...
  if (matches >= 2) {
    ...
    let multiplier = 1
    if (gameStore.state.chain >= 2)
      multiplier = gameStore.state.chain
    ...
    for (const pos in gameStore.state.matches) {
      if (Object.prototype.hasOwnProperty.call(gameStore.state.matches, pos)) {
        ...
        const xMovement = x - Number.parseInt(localX)
        const yMovement = y - Number.parseInt(localY)
